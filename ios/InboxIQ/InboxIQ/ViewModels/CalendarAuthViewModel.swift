...
final class CalendarAuthViewModel: ObservableObject {
    ...
    private func getBackendUserId() -> UUID? {
        guard let userIdString = UserDefaults.standard.string(forKey: "backend_user_id"),
    ...
    func checkStatus(context: NSManagedObjectContext) async {
        ...
        guard let userId = getBackendUserId() else {
            error = AppError.auth("Not logged in - no backend user ID")
            return
        }

        do {
            let status = try await CalendarService.shared.checkStatus(userId: userId)
    ...
    func startAuth(context: NSManagedObjectContext) async {
        ...
        do {
            let response = try await CalendarService.shared.initiateAuth(userId: userId)
