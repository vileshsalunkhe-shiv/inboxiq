...
final class CalendarService {
    ...
    func handleCallback(code: String, state: String) async throws -> CalendarCallbackResponse {
        ...
        var components = URLComponents(url: Constants.apiBaseURL.appendingPathComponent(APIPath.calendarCallback), resolvingAgainstBaseURL: false)!
    ...
    func fetchEvents(userId: UUID, maxResults: Int = 10) async throws -> [CalendarEventPayload] {
        ...
        var components = URLComponents(url: Constants.apiBaseURL.appendingPathComponent(APIPath.calendarEvents), resolvingAgainstBaseURL: false)!
        components.queryItems = [
